...
function commonsPhoto(file){return 'https://commons.wikimedia.org/wiki/Special:Redirect/file/'+encodeURIComponent(file)+'?width=1400';}
function partPhoto(p){var ref=partPhotoLibrary[p.id]||partCategoryPhotos[p.cat]||partCategoryPhotos.engine;return '<figure class="part-photo" id="part-photo-'+h(p.id)+'"><a class="part-photo-link" href="'+ref[2]+'" target="_blank" rel="noopener" title="Open original image and license"><img src="'+commonsPhoto(ref[0])+'" alt="'+h(p.name)+' reference photo" loading="lazy" referrerpolicy="no-referrer"></a><figcaption><span class="part-photo-caption"><i class="ph ph-camera"></i> Loading live part photo…</span><a class="part-photo-source" href="'+ref[2]+'" target="_blank" rel="noopener">Photo source / license <i class="ph ph-arrow-square-out"></i></a></figcaption></figure>';}
var partImageRequest=0;
function loadLivePartPhoto(p){var requestId=++partImageRequest;var fig=document.getElementById('part-photo-'+p.id);if(!fig)return;fetch('/api/part_image?q='+encodeURIComponent(p.name)).then(function(r){if(!r.ok)throw new Error('lookup failed');return r.json()}).then(function(data){if(requestId!==partImageRequest||!data.ok||!data.result)return;var current=document.getElementById('part-photo-'+p.id);if(!current)return;var img=current.querySelector('img'),link=current.querySelector('.part-photo-link'),src=current.querySelector('.part-photo-source'),cap=current.querySelector('.part-photo-caption');if(img){img.src=data.result.image_url;img.alt=p.name+' real-world reference photo';}if(link)link.href=data.result.source_url;if(src)src.href=data.result.source_url;if(cap)cap.innerHTML='<i class="ph ph-camera"></i> Live web reference · '+h(data.result.title||p.name);}).catch(function(){var current=document.getElementById('part-photo-'+p.id);if(!current)return;var cap=current.querySelector('.part-photo-caption');if(cap)cap.innerHTML='<i class="ph ph-camera"></i> Curated reference photo';});}
...
function selectPart(id){var p=partsDB.find(function(x){return x.id===id});if(!p)return;selectedPart=id;var d=document.getElementById('part-detail');if(!d)return;d.dataset.ready='1';d.innerHTML='<div class="part-hero"><div><span class="eyebrow">'+h(p.cat.toUpperCase())+' · '+h(p.loc.toUpperCase())+'</span><h2>'+h(p.name)+'</h2><p>'+h(p.what)+'</p></div><button class="part-3d-btn" onclick="showPartOnTruck(\''+p.id+'\')"><i class="ph ph-cube"></i> View in 3D Lab</button></div><div class="part-visual-head"><div><span class="part-label">PART PHOTO</span><strong>What it actually looks like</strong></div><small>Real-world reference photo · open the source for attribution/license and model-specific context</small></div>'+partPhoto(p)+'<div class="part-info-grid"><article><span class="part-label">HOW IT WORKS</span><p>'+h(p.works)+'</p></article><article><span class="part-label">COMMON ISSUES</span><ul>'+p.issues.map(function(x){return '<li>'+h(x)+'</li>'}).join('')+'</ul></article><article><span class="part-label">QUICK CHECKS</span><ol>'+p.checks.map(function(x){return '<li>'+h(x)+'</li>'}).join('')+'</ol></article><article class="part-fix"><span class="part-label">FIX / NEXT ACTION</span><p>'+h(p.fix)+'</p></article></div><a class="part-source" href="'+p.url+'" target="_blank" rel="noopener"><i class="ph ph-arrow-square-out"></i><span><strong>Source / further reading</strong><small>'+h(p.source)+'</small></span></a>';loadLivePartPhoto(p);renderPartsManual(document.getElementById('parts-search')?document.getElementById('parts-search').value:'');}
