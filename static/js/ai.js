...
function editAIKnowledge(id){var k=aiKnowledgeItems.find(function(x){return String(x.id)===String(id)});if(!k)return;document.getElementById('ai-edit-id').value=k.id;document.getElementById('ai-edit-title').value=k.title||'';document.getElementById('ai-edit-content').value=k.content||'';document.getElementById('ai-edit-tags').value=(k.tags||[]).join(', ');document.getElementById('ai-edit-modal').hidden=false;document.body.classList.add('modal-open')}
