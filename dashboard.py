...
@media(max-width:768px){
  ...
  .sidebar{
    position:fixed;left:0;top:0;height:100%!important;
    width:80vw;max-width:280px;
    transform:translateX(-100%);
    z-index:200;
    box-shadow:4px 0 32px rgba(44,36,22,.25);
  ...
  .sidebar-overlay{
    ...
    background:rgba(44,36,22,.5);z-index:199;
  ...
  .topbar-right .badge-btn i{font-size:15px}
  ...
  /* ── Search ── */
  .search-wrap input{font-size:14px;padding:10px 14px 10px 40px}

  /* ── Nav items: bigger touch targets ── */
  .nav-item{padding:12px 12px;font-size:13px}
  .nav-item i{font-size:16px}
  .nav-group-header{padding:12px 12px;font-size:13px}
  .nav-sub{padding-left:32px!important;font-size:12px!important}
  ...
  #heatmap-container table{min-width:700px}
}
...
@media(max-width:480px){
  ...
  /* Hide less important topbar buttons */
  .topbar-right .badge-btn:nth-child(1),
  .topbar-right .badge-btn:nth-child(2){display:none}
  .topbar-right .badge-btn:last-child{display:flex}
}
...
def api_export_pdf():
    ...
    try:
        ...
        # Colors matching Kurtex theme
        ACCENT   = colors.HexColor("#C17B3F")
        ACCENT2  = colors.HexColor("#A0622A")
        ...
        title_style  = style("title",  fontSize=22, textColor=TEXT, fontName="Helvetica-Bold", spaceAfter=2)
        sub_style    = style("sub",    fontSize=10, textColor=MUTED, fontName="Helvetica", spaceAfter=2)
        label_style  = style("label",  fontSize=8,  textColor=MUTED, fontName="Helvetica", alignment=TA_CENTER)
        val_style    = style("val",    fontSize=20, textColor=ACCENT, fontName="Helvetica-Bold", alignment=TA_CENTER)
        sec_style    = style("sec",    fontSize=11, textColor=TEXT, fontName="Helvetica-Bold", spaceBefore=8, spaceAfter=4)
        ...
        logo_img = None
        try:
            logo_bytes = base64.b64decode(KURTEX_LOGO_B64)
            logo_buf = io.BytesIO(logo_bytes)
            logo_img = Image(logo_buf, width=0.7*inch, height=0.7*inch)
        except: pass

        now_str = datetime.now().strftime("%B %d, %Y — %I:%M %p ET")
        header_data = [[
            logo_img or "",
            Paragraph("<b>Kurtex Dashboard</b><br/>Truck Maintenance Command Center", style("hd", fontSize=13, textColor=TEXT, fontName="Helvetica-Bold", leading=16)),
            Paragraph(f"<font color='#8C7B6B'>{now_str}</font>", style("dt", fontSize=9, textColor=MUTED, fontName="Helvetica", alignment=TA_RIGHT)),
        ]]
        header_table = Table(header_data, colWidths=[0.9*inch, 5.5*inch, 3.2*inch])
        ...
        # Stats cards row
        def stat_cell(label_txt, value, color):
            return [Paragraph(label_txt, label_style), Paragraph(str(value), style("sv", fontSize=22, textColor=color, fontName="Helvetica-Bold", alignment=TA_CENTER))]
        ...
        stats_table.setStyle(TableStyle([
            ...
            ("ROWBACKGROUNDS", (0,0), (-1,-1), [BG_LIGHT, BG_ROW]),
        ...
        # Leaderboard + Cases table side by side
        ...
        for i, (name, count) in enumerate(agent_counts.most_common(8)):
            lb_data.append([Paragraph(f"<b>{medals[i] if i<len(medals) else str(i+1)+'.'}</b>  {name}  <font color='#C17B3F'><b>{count}</b></font>", style("lbr", fontSize=9, textColor=TEXT, fontName="Helvetica"))])
        ...
        # Cases table
        status_colors = {"done": GREEN, "missed": RED, "assigned": YELLOW, "reported": PURPLE, "open": colors.HexColor("#2E6EA6")}
        cases_data = [[
        ...
        combo.setStyle(TableStyle([("VALIGN",(0,0),(-1,-1),"TOP"),("LEFTPADDING",(0,0),(-1,-1),0),("RIGHTPADDING",(0,0),(-1,-1),0),("TOPPADDING",(0,0),(-1,-1),0)]))
        story.append(combo)

        # Footer
        story.append(Spacer(1, 8))
        story.append(HRFlowable(width="100%", thickness=0.5, color=colors.HexColor("#E2D9CC")))
        story.append(Paragraph(f"Generated by Kurtex Dashboard · {now_str} · kurtex-alert-bot-production.up.railway.app",
